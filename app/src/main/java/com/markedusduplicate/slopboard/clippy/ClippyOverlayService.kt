 ...
 * the keyboard. Tapping the mascot screenshots the current screen (via the accessibility service's
 * [ScreenshotCapturer]) and asks the on-device multimodal LLM ([LlmEngine]) for a snarky one-liner,
 * shown in a speech bubble — the same capture→infer path the keyboard's Vision feature uses.
 *
...
class ClippyOverlayService :
    ...
    private val state = MutableStateFlow<ClippyState>(ClippyState.Idle)
    ...
    override fun onCreate() {
        ...
        val view = ClippyComposeView(
            ...
            onTap = ::onTap,
            onDrag = ::onDrag,
            onDismiss = { state.value = ClippyState.Idle },
        ).also { view ->
    ...
    /** Screenshot the current screen and ask the LLM for a one-line roast. */
    private fun onTap() {
        tapJob?.cancel()
        tapJob = scope.launch {
